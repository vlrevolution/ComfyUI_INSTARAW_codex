...
def load_locations_data():
    if not LOCATIONS_FILE.exists():
        ...
        default_data = {
          ...
          "Europe": { "France": [ { "city": "Paris", "lat": [48.8156, 48.9021], "lon": [2.2241, 2.4699] } ] }
        }
...
class INSTARAW_SynthesizeAuthenticMetadata:
    ...
    def get_location_choices(cls):
        ...
        if cls._location_choices is None:
            special_options = ["From Profile", "Synthesize Random (Any City)", "Synthesize Random (US City)"]
            ...
            cls._location_choices = special_options + random_country_options + sorted(specific_city_options)
    ...
    def INPUT_TYPES(cls):
        ...
        scene_types = ["From Profile", "Synthesize Random", "Daylight (Bright)", "Daylight (Cloudy)", "Golden Hour", "Indoors", "Night"]
        ...
        return {
            ...
            "optional": {
                "location_synthesis": (cls.get_location_choices(), {"default": "Synthesize Random (US City)"}),
                "datetime_synthesis": (["From Profile", "Synthesize Random"], {"default": "Synthesize Random"}),
    ...
    def get_random_city_from_country(self, country_name, rng):
        cities_in_country = []
    ...
    def synthesize_and_save(self, image: torch.Tensor, profile_path: str, seed: int,
                   location_synthesis="Synthesize Random (US City)", datetime_synthesis="Synthesize Random", 
                   start_date="", end_date="", scene_type="Synthesize Random"):
        
        if not EXIFTOOL_AVAILABLE: raise Exception("ExifTool is not installed.")
        ...
        for i in range(image.shape[0]):
            ...
            # --- THE DEFINITIVE FILENAME FIX ---
            # Call the helper function INSIDE the loop for each image.
            # We discard the last return value because it's not the extension.
            full_output_folder, filename, counter, subfolder, _ = folder_paths.get_save_image_path("IMG", output_dir)
            
            # Manually construct the filename with the correct, hardcoded extension.
            ...
            img_pil = Image.fromarray((np.clip(img_np, 0.0, 1.0) * 255.0).astype(np.uint8), 'RGB')
            
            json_path = f"{profile_path}.json" if not profile_path.endswith('.json') else profile_path
            with open(json_path, 'r', encoding='utf-8') as f:
                metadata_library = json.load(f)
            selected_profile = rng.choice(metadata_library).copy()

            width, height = img_pil.size
            tags_to_align = {
                "EXIF:ExifImageWidth": width, "EXIF:ImageWidth": width,
                "EXIF:ExifImageHeight": height, "EXIF:ImageHeight": height,
                "XMP:RegionAppliedToDimensionsW": width, "XMP:RegionAppliedToDimensionsH": height,
            }
            for tag, value in tags_to_align.items():
                if tag in selected_profile:
                    selected_profile[tag] = value

            bounds = None
            if location_synthesis == "From Profile": pass
                ...
                country_name = location_synthesis[19:-1]
                bounds = self.get_random_city_from_country(country_name, rng)
            else:
                    ...
                    if country_name in countries:
                        for city_data in countries[country_name]:
                            if city_data['city'] == city_name:
                                bounds = city_data; break
                
            if bounds:
                lat = rng.uniform(bounds['lat'][0], bounds['lat'][1])
                lon = rng.uniform(bounds['lon'][0], bounds['lon'][1])
                selected_profile.update({
                    "EXIF:GPSLatitude": abs(lat), "EXIF:GPSLatitudeRef": "N" if lat >= 0 else "S",
                    "EXIF:GPSLongitude": abs(lon), "EXIF:GPSLongitudeRef": "E" if lon >= 0 else "W",
                    "EXIF:GPSAltitude": rng.uniform(10, 200), "EXIF:GPSAltitudeRef": 0,
                    "EXIF:GPSImgDirection": rng.uniform(0, 359.9), "EXIF:GPSImgDirectionRef": "T"
                })
            ...
            if datetime_synthesis == "Synthesize Random":
                try:
                    ...
                    selected_profile.update({
                        "EXIF:DateTimeOriginal": exif_date_str, "EXIF:CreateDate": exif_date_str, "EXIF:ModifyDate": exif_date_str,
            ...
            if actual_scene_type != "From Profile":
                ...
                if actual_scene_type == "Daylight (Bright)":
                    settings = {"EXIF:ISO": rng.choice([25, 32, 50]), "EXIF:ExposureTime": 1 / rng.randint(1000, 4000), "EXIF:FNumber": 2.2}
                elif actual_scene_type == "Night":
                    settings = {"EXIF:ISO": rng.choice([800, 1600, 2500]), "EXIF:ExposureTime": 1 / rng.randint(15, 60), "EXIF:FNumber": 1.8}
                selected_profile.update(settings)
                
            if "MakerNotes:PhotoIdentifier" in selected_profile:
            ...
            try:
                ...
                if has_icc_profile:
                    icc_filename = selected_profile.pop(icc_profile_key)
                    if json_path:
                        profile_dir = os.path.dirname(os.path.abspath(json_path))
                        full_icc_path = os.path.join(profile_dir, icc_filename)
                        if os.path.exists(full_icc_path): icc_profile_path = full_icc_path
                ...
                if icc_profile_path: command.append(f"-ICC_Profile<={icc_profile_path}")
                command.extend(["-overwrite_original", "-ignoreMinorErrors", temp_filepath])
                
                subprocess.run(command, capture_output=True, text=True, check=False, encoding='utf-8', errors='ignore')
            ...
            if i == 0: final_filepath_for_return = final_filepath
            results_for_ui.append({"filename": os.path.basename(final_filepath), "subfolder": subfolder, "type": "output"})
